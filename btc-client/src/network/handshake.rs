...
pub fn perform_handshake(
    ...
    send_message(
        stream,
        magic,
        NetworkMessage::Version(build_version_msg(peer_addr)),
    )?;
    ...
    while peer_version.is_none() || !got_verack {
        let raw = recv_message(stream)?;
        match raw.payload().clone() {
            NetworkMessage::Version(v) => {
                println!(
                    "[RECV] version | agent='{}' height={}",
                    v.user_agent, v.start_height
                );
    ...
    println!("[INFO] Complete Handshake ✓\n");
